    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
                ...
                />
                <StatCard label="Monthly Rent" value={lease ? formatCurrency(lease.rent_amount) : '—'} />
                <StatCard label="Unpaid Payments" value={unpaidCount} accent={unpaidCount > 0 ? 'red' : 'green'} />
                <StatCard label="Open Tickets" value={openTickets} accent={openTickets > 0 ? 'yellow' : 'green'} />
            </div>
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const managed = allProps.filter(
                    (p) => !p.leases?.some((l) => l.status === 'active' && l.tenant?.id === user?.id),
                )
        ...
        return (
            <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <p className="text-sm text-gray-500 mt-2">
                    No managed properties yet. Ask your landlord to assign you.
                </p>
            </div>
        )
    ...
    return (
        <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Managed Properties" value={properties.length} subtitle={`${occupiedCount} occupied`} />
                <StatCard
                    label="Open Tickets"
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 mt-0.5">
                                                {ticket.property?.address?.split(',')[0] || '—'} ·{' '}
